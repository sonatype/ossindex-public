    ...
    implements OssindexClient
{
    ...
    public OssindexClientImpl(final OssindexClientConfiguration config, final Transport transport, final Marshaller marshaller) {
        checkNotNull(config);
    ...
    private Map<PackageUrl,ComponentReport> doRequestComponentReports(final List<PackageUrl> coordinates) throws Exception {
        ...
        checkState(reports.size() == coordinates.size(), "Result size mismatch; expected: %s, have: %s", coordinates.size(), reports.size());
